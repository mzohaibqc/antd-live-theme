...
/*
  ...
  - outputFile - output file containing only color related css rules which needs to be imported in `index.html`
*/
const stylesDir = path.resolve(__dirname, './src/styles/');
const indexFile = path.join(stylesDir, './index.less');
const varFile = path.join(stylesDir, './variables.less');
const themeFile = path.join(stylesDir, './theme.less');
const outputFile = path.join(__dirname, './public/color.less');

const themeVars = getLessVars(themeFile);
const themeVarNames = Object.keys(themeVars);
...
/*
  This method takes less input as string and compiles into css.
*/
...
/*
  This method reads a less file and create an object with keys as variable names 
...
/*
  This method take primary color palette name and returns @primary-color dependent value
  .e.g 
  Input: @primary-1
...
/*
  This method takes color string as input and return true if string is a valid color otherwise returns false.
  e.g.
...
function generateCss() {
  ...
  bundle({
    ...
    css = `${colorsLess}\n${css}`;
    return render(css).then(({ css }) => [css, mappings, colorsLess]);
  }).then(([css, mappings, colorsLess]) => {
    css = css.replace(/(\/.*\/)/g, '');
    const regex = /.(?=\S*['-])([.a-zA-Z0-9'-]+)\ {\n\ \ color:\ (.*);/g;
    themeCompiledVars = getMatches(css, regex);
    content = `${content}\n${colorsLess}`;
    return render(content).then(({ css }) => [css, mappings, colorsLess]);
  }).then(([css, mappings, colorsLess]) => {
